...
function ajax_comment_callback(){
    ...
    if ( $user->exists() ) {
        ...
        if ( get_option('comment_registration') || 'private' == $status )
            ajax_comment_err('抱歉，您必须登录到发表评论');
    }
    $comment_type = '';
    if ( get_option('require_name_email') && !$user->exists() ) {
        if ( 6 > strlen($comment_author_email) || '' == $comment_author )
            ajax_comment_err( 'Error: 请填写所需的字段（名称，电子邮件）' );
        elseif ( !is_email($comment_author_email))
            ajax_comment_err( 'Error: 请输入有效的电子邮件地址' );
    }
    if ( '' == $comment_content )
        ajax_comment_err( 'Error: 请输入评论内容' );
    $dupe = "SELECT comment_ID FROM $wpdb->comments WHERE comment_post_ID = '$comment_post_ID' AND ( comment_author = '$comment_author' ";
    if ( $comment_author_email ) $dupe .= "OR comment_author_email = '$comment_author_email' ";
    $dupe .= ") AND comment_content = '$comment_content' LIMIT 1";
    if ( $wpdb->get_var($dupe) ) {
        ajax_comment_err('发现重复评论；它看起来好像你已经说了');
    }
    ...
    <li <?php comment_class(); ?>>
       <div id="comment-<?php comment_ID(); ?>" class="comment-body" style="border-bottom:0;">
		...
		<div class="comment-head">
			<?php printf( __( '<span class="name">%s</span>'), get_comment_author_link() ); ?></span>  <span class="date">發布於 - <?php echo $cmt_time ;?></span> <?php comment_text() ?>
